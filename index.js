...
const _app = {
  ...
  game_loop : async () => {
    ...
    let user_in = await _app._internals.prompt(`Player ${_app._player_turn+1} enter a coord x,y to place your ${_app._player_list[_app._player_turn]} or enter 'q' to give up: `);
    let user_coord = user_in.match(/^(\d+),(\d+)$/);
    ...
    if (user_in === 'q') {
      ...
      if (index > -1) _app._internals.make_move(index);
    }
  ...
  _internals : {
    ...
    make_move : (index) => {
      if (!_app._internals.check_avail(index)) {
        console.error(`Invalid move. The space is already occuped with "${_app._board_state[index]}"`);
        return false;
      }
